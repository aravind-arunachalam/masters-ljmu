...
from dataset_gqa import GQAFeatureDataset, ObjectSpatialFeature, tfidf_from_questions_gqa
...
if __name__ == '__main__':
    ...
    if args.dataset == "vqa_cp":
        ...
        osf_object = ObjectSpatialFeature(args.data_folder)
        val_dset = GQAFeatureDataset(
                    'val_balanced', dictionary, osf_object, args.relation_type, adaptive=args.adaptive, 
                    pos_emb_dim=args.imp_pos_emb_dim, dataroot=args.data_folder)
        train_dset = GQAFeatureDataset(
                    'train_balanced', dictionary, osf_object, args.relation_type, adaptive=args.adaptive, 
                    pos_emb_dim=args.imp_pos_emb_dim, dataroot=args.data_folder)
    else:
    ...
    if args.dataset == "gqa" and args.tfidf:
        tfidf, weights = tfidf_from_questions_gqa(['train_balanced', 'val_balanced', 'test_balanced'],
                                              dictionary)
    elif args.tfidf:
    ...
    if args.use_both and args.dataset == "vqa":
        ...
        train_loader = DataLoader(train_dset, batch_size, shuffle=True,
                                  num_workers=32, collate_fn=trim_collate)
